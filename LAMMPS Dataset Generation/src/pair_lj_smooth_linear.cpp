...
void PairLJSmoothLinear::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      jtype = type[j];
//start
      myfile.open ("Compute_CSV_input.txt",std::ofstream::app);
      myfile  << rsq <<","<< lj1[itype][jtype] <<","<< lj2[itype][jtype]  <<","<< dljcut[itype][jtype]  <<","<< factor_lj <<","<< delx <<","<< dely  <<","<<delz  <<","<< cut[itype][jtype]  << "\n";
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        myfile1.open ("Compute_CSV_output.txt",std::ofstream::app);
        myfile1  << f[i][0] <<","<< f[i][1] <<","<< f[i][2]  <<","<< f[j][0] <<","<< f[j][1] <<","<< f[j][2] << "," <<  evdwl  << "\n";
